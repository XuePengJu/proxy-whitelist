// ============================================================
// ProxyMate - 规则管理页逻辑
// 全部规则统一列表（文件规则 + 手动规则），支持添加/删除/导入/复制
// ============================================================
...
function renderAll() {
  ...
  if (!total) {
    els.listMeta.textContent = "暂无规则，添加或导入后生效";
  ...
  els.listMeta.textContent = `文件规则 ${fileCount} 条（📄 rules/cn-direct.txt）+ 手动 ${currentManualRules.length} 条 · 命中即直连`;
